// Code generated by ent, DO NOT EDIT.

package gen

import (
	"context"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/toheart/goanalysis/internal/data/ent/runtime/gen/paramstoredata"
	"github.com/toheart/goanalysis/internal/data/ent/runtime/gen/predicate"
)

// ParamStoreDataDelete is the builder for deleting a ParamStoreData entity.
type ParamStoreDataDelete struct {
	config
	hooks    []Hook
	mutation *ParamStoreDataMutation
}

// Where appends a list predicates to the ParamStoreDataDelete builder.
func (psdd *ParamStoreDataDelete) Where(ps ...predicate.ParamStoreData) *ParamStoreDataDelete {
	psdd.mutation.Where(ps...)
	return psdd
}

// Exec executes the deletion query and returns how many vertices were deleted.
func (psdd *ParamStoreDataDelete) Exec(ctx context.Context) (int, error) {
	return withHooks(ctx, psdd.sqlExec, psdd.mutation, psdd.hooks)
}

// ExecX is like Exec, but panics if an error occurs.
func (psdd *ParamStoreDataDelete) ExecX(ctx context.Context) int {
	n, err := psdd.Exec(ctx)
	if err != nil {
		panic(err)
	}
	return n
}

func (psdd *ParamStoreDataDelete) sqlExec(ctx context.Context) (int, error) {
	_spec := sqlgraph.NewDeleteSpec(paramstoredata.Table, sqlgraph.NewFieldSpec(paramstoredata.FieldID, field.TypeInt64))
	if ps := psdd.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	affected, err := sqlgraph.DeleteNodes(ctx, psdd.driver, _spec)
	if err != nil && sqlgraph.IsConstraintError(err) {
		err = &ConstraintError{msg: err.Error(), wrap: err}
	}
	psdd.mutation.done = true
	return affected, err
}

// ParamStoreDataDeleteOne is the builder for deleting a single ParamStoreData entity.
type ParamStoreDataDeleteOne struct {
	psdd *ParamStoreDataDelete
}

// Where appends a list predicates to the ParamStoreDataDelete builder.
func (psddo *ParamStoreDataDeleteOne) Where(ps ...predicate.ParamStoreData) *ParamStoreDataDeleteOne {
	psddo.psdd.mutation.Where(ps...)
	return psddo
}

// Exec executes the deletion query.
func (psddo *ParamStoreDataDeleteOne) Exec(ctx context.Context) error {
	n, err := psddo.psdd.Exec(ctx)
	switch {
	case err != nil:
		return err
	case n == 0:
		return &NotFoundError{paramstoredata.Label}
	default:
		return nil
	}
}

// ExecX is like Exec, but panics if an error occurs.
func (psddo *ParamStoreDataDeleteOne) ExecX(ctx context.Context) {
	if err := psddo.Exec(ctx); err != nil {
		panic(err)
	}
}
