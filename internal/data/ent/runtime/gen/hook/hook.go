...
// The GoroutineTraceFunc type is an adapter to allow the use of ordinary
// function as GoroutineTrace mutator.
type GoroutineTraceFunc func(context.Context, *gen.GoroutineTraceMutation) (gen.Value, error)

// Mutate calls f(ctx, m).
func (f GoroutineTraceFunc) Mutate(ctx context.Context, m gen.Mutation) (gen.Value, error) {
	if mv, ok := m.(*gen.GoroutineTraceMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f TraceDataFunc) Mutate(ctx context.Context, m gen.Mutation) (gen.Value, error) {
	if mv, ok := m.(*gen.TraceDataMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *gen.TraceDataMutation", m)
}
