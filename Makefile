...
package-linux: sync-frontend
	GOOS=linux GOARCH=amd64 go build -ldflags "-X main.Version=$(VERSION)" -o goanalysis ./cmd
	mkdir -p release
...
package-windows: sync-frontend
	GOOS=windows GOARCH=amd64 go build -ldflags "-X main.Version=$(VERSION)" -o goanalysis.exe ./cmd
	mkdir -p release
